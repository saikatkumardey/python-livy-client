...
class _LivyConnection(object):
    ...
    _HTTP_TIMEOUT_CONF = 'livy.client.http.connection.timeout'
    _HTTP_TIMEOUT = 180
    ...
    def __init__(self, uri, config):
        ...
        self._http_connnection_timeout= \
            config.getint('env',self._HTTP_TIMEOUT_CONF) \
            if config.has_option('env',self._HTTP_TIMEOUT_CONF) else self._HTTP_TIMEOUT
        ...
        try:
            with self.lock:
                ...
                return self._requests.request(method, request_url,
                    timeout=self._http_connnection_timeout, headers=local_headers, files=files,
